...
@task(max_retries=4, retry_delay=timedelta(seconds=2))
...
with Flow("extract_market_data") as flow_market:
    # Define the flow input parameters
    index_name = Parameter("index", default="DJI")
    root_market_data = Parameter("root_folder", "~/git/market_data/download_flow")
...
flow_market.schedule = Schedule(clocks=[CronClock("0 0 * * *")])
flow_market.run_config = LocalRun(labels=["local"])
flow_market.executor = DaskExecutor(address="tcp://localhost:8786")
